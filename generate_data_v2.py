...
def main(file):
    ...
    with open(GOOD_COMMENTS_FILE, 'r') as coms:
        GOOD_COMMENTS = [com.replace('\n', '') for com in coms]
        
    with open(BAD_COMMENTS_FILE, 'r') as coms:
        BAD_COMMENTS = [com.replace('\n', '') for com in coms]
    ...
    for i in range(1, TRIPS_NUM + 1):
        ...
        if i % BATCH_SIZE == 0:
            TRIPS = pd.DataFrame(TRIPS, columns=['driver', 'client', 'start_point', 'end_point', 'distance', 'start_date', 'end_date', 'driver_rate', 'driver_comment', 'driver_feedback', 'client_rate', 'client_feedback', 'cost'])
            file_path = 'data_small.csv'
            with open(file_path, mode='a', newline='') as file:
                if flag:
                    TRIPS.to_csv(file, header=True, index=False)
                    flag = False
                else:
                    TRIPS.to_csv(file, header=False, index=False)
            TRIPS = []        
